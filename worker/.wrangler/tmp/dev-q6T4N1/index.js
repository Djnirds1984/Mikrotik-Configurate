...
// .wrangler/tmp/bundle-eV1EW0/strip-cf-connecting-ip-header.js
...
// src/index.js
var supabaseUrl = "https://mivtfrprigvznzdtjiqz.supabase.co";
var supabaseServiceKey = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im1pdnRmcnByaWd2em56ZHRqaXF6Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzAzMTQ2MTcsImV4cCI6MjA4NTg5MDYxN30.jnzjHqcg5hS_Vzlntgpu3BWZJXULbZaz1CmgbFwTphI";
...
__name(handleRouterApi, "handleRouterApi");
...
__name(handleVoucherApi, "handleVoucherApi");
...
__name(handleMikrotikApi, "handleMikrotikApi");
async function testRouterConnection(routerId) {
  try {
    ...
    if (testResult.success) {
      await supabase.from("routers").update({ status: "online", last_seen: (/* @__PURE__ */ new Date()).toISOString() }).eq("id", routerId);
      ...
      await supabase.from("routers").update({ status: "offline", last_seen: (/* @__PURE__ */ new Date()).toISOString() }).eq("id", routerId);
      return new Response(JSON.stringify({
        success: false,
...
__name(testRouterConnection, "testRouterConnection");
async function fetchRouterConfig(routerId) {
  try {
    ...
    await supabase.from("router_configs").insert({
      router_id: routerId,
      config_data: config,
...
__name(fetchRouterConfig, "fetchRouterConfig");
...
__name(bulkRouterOperation, "bulkRouterOperation");
async function generateVoucherPDF(request) {
  try {
    const { voucher_ids } = await request.json();
    const { data: vouchers, error } = await supabase.from("vouchers").select("*, routers(name)").in("id", voucher_ids);
    if (error) {
...
__name(generateVoucherPDF, "generateVoucherPDF");
...
__name(generateVoucherHTML, "generateVoucherHTML");
  ...
  try {
    ...
    for (const router of routers) {
      try {
        ...
        results.push({ router_id: router.id, success: false, error: error2.message });
      }
    }
    return new Response(JSON.stringify({ success: true, results }), {
      headers: {
...
__name(syncAllRouters, "syncAllRouters");
...
__name(mikrotikApiCall, "mikrotikApiCall");
...
// .wrangler/tmp/bundle-eV1EW0/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-eV1EW0/middleware-loader.entry.ts
